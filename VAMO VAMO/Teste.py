...
def Tela_CadastroAlimento(root, email, refeicao):
    ...
    def adicionar_alimento():
        try:
            ...
            historico_alimento = HistoricoAlimentos()
            
            if historico_alimento.salvaAlimento(email, refeicao, descricao_alimento, alimento.nutrientes):
                error_label.config(text="Alimento adicionado com sucesso!", fg="green")
            else:
                error_label.config(text="Erro ao adicionar alimento!", fg="red")
...
def Tela_Historico_Insulina(root, email_usuario):
    ...
    tk.Label(frame, text="Dosagem Insulina",font=("Helvetica", 16), bg="#0493b3").place(x=centro_x, y=50, anchor="center")
