...
class HistoricoRefeicao:
    ...
    def salvaRefeicao(self, usuario, refeicao, nutrientes, insulina): 
        ...
        if resposta_refeicao.data and len(resposta_refeicao.data) > 0:
            ...
            print("Erro: Refeição não encontrada no banco de dados.")
            return False
        ...
        if response.data:  
            print("Refeição salva com sucesso no banco de dados.")
            return True

        print("Erro ao salvar a refeição no banco de dados.")
    ...
    def mostraHistorico(self, data, refeicao, usuario):
        ...
        for i, item in enumerate(dados, 1):
            linhas_formatadas.append(f"Item {i}:")
